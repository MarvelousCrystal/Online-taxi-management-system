...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener{
    private EditText new_number;//注册页面的用户名输入框
     ...
     */
    private void init() {
        new_number=(EditText)findViewById(R.id.newnumber_text);//创建注册页面的用户名输入框对象
    ...
    private void register(){
        //获取输入的用户名与密码
        String numbertext=new_number.getText().toString().trim();
        String passwordtext=new_password.getText().toString().trim();
        //定义用户名和密码规则
        String numberregex = "[0-9]{5}";
        String passwordregex = "[0-9]{5}";
        //校验用户名
        if (!(numbertext.matches(numberregex))) {
            ...
            Toast.makeText(RegisterActivity.this,"用户账号格式不对，请输入5位单词字母",Toast.LENGTH_LONG).show();
            //清空用户名框文本，并把输入光标放回到输入框
            this.new_number.setText("");
        ...
        //调用注册功能
        Manager manager=new Manager(numbertext,passwordtext);
        Manager_InterfaceImple loginImpl=new Manager_InterfaceImple();
        boolean result=loginImpl.register(this,manager);//判断是否注册成功
        if(result){
